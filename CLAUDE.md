...
- **Page transitions** — a "page push" illusion, not just a covering panel. All real page content (everything except `nav`, the `.mnav` mobile overlay, `.site-grain`, `.page-sweep`, and the index-only loader) is wrapped in `.page-view`. On exit, `.page-view` slides -4% and dims (0.38s) while `.page-sweep` — a gradient panel with the vector wordmark centred, gold hairline edges — slides in from the right (`body.page-leaving`, 0.52s), then navigates. The next page reads a `sessionStorage('pt')` flag in a `<head>` script, starts fully covered+hidden (`html.pt-in`: `.page-sweep` at rest, `.page-view` at `opacity:0` + `clip-path: inset(0 0 0 100%)`), then both animate open together (`html.pt-in.pt-go`: panel exits left over 0.68s, `.page-view` reveals via `opacity`+`clip-path` over ~0.6s) — the panel retreating "opens" the new page rather than just uncovering a static one. bfcache via `pageshow`. **Entry deliberately never uses `transform`** on `.page-view` — a transform would make it a new containing block for its `position:fixed` descendant (`.chapter-rail` on index), skewing every `offsetTop` measurement (rail's `measure()`, ScrollTrigger setup) that runs during page load; `opacity`/`clip-path` don't have that side effect. (A pure-CSS View Transitions version was tried 2026-06-12 and reverted — it didn't fire reliably in the client's Chrome — this hand-rolled class-toggle approach is unrelated to that API and unaffected by its reliability issues.) Disabled under reduced motion.
...
- **Site loader** (`index.html` only, every visit) — fullscreen green-wall video (`loader-wall.mp4`, 470KB) with the same vector wordmark used in the nav (`.loader-logo.logo-svg` — needs both classes: `logo-svg` is what makes the global CSS hide the layer-back/layer-stroke shadow copies; without it they render as opaque black text) + accent progress line; min 1.9s, max 4.5s, exits by **evaporating** (opacity + `blur(18px)` + slight scale/upward drift, 1.15s). Skipped under reduced-motion.
